...
class MemoryModel(torch.nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        :param beta:, float, the time decay weight of PINT for aggregating neighbor nodes
        :param num_hop:, int, the maximum hop of the PINT's maintained temporal walk matrices
        """
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
        ...
        if edges_are_positive:
            ...
            # updating the temporal walk matrices
            if self.model_name == 'PINT':
...
class MatrixMemory(nn.Module):
    def __init__(self, num_node: int, num_hop: int, device: str):
        """
        Module to maintain the temporal walk matrices
        :param num_node: int, the number of nodes
        :param num_hop: int, the maximum hop of the maintained temporal walk matrices
        :param device: str, device
        """
    ...
    def reset_memory(self):
        """
        reinitialize the temporal walk matrices
        """
        self.matrix.data[:, :, 0] = torch.eye(self.num_node)

    def update(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray):
        """
        update the temporal walk matrices after observing a batch of interaction
    ...
    def get_memory(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray):
        """
        get the relative encodings for the given node pairs
        :param src_node_ids: np.ndarray, shape (batch,), source node ids
        :param dst_node_ids: np.ndarray, shape (batch,),destination node ids
        :return:
        """
    ...
    def backup_memory(self):
        """
        backup the maintained temporal walk matrices
        :return:
        """
    ...
    def reload_memory(self, data):
        """
        reload the temporal walk matrices
        :param data:
        """
    ...
    def __init__(self, node_raw_features: torch.Tensor, edge_raw_features: torch.Tensor,
        ...
        self.merge_layers = nn.ModuleList(
            [MergeLayer(input_dim1=self.node_feat_dim + self.time_feat_dim, input_dim2=self.node_feat_dim,
                        hidden_dim=self.node_feat_dim, output_dim=self.node_feat_dim) for _ in
             range(num_layers)])
...
class PINTEmbedding(nn.Module):
    def __init__(self, node_raw_features: torch.Tensor, edge_raw_features: torch.Tensor,
                 neighbor_sampler: NeighborSampler, time_encoder: TimeEncoder, num_layers: int,
                 node_feat_dim: int, edge_feat_dim: int, time_feat_dim: int, matrix_memory: MatrixMemory, device: str,
                 beta: float, r_dim: int):
        """
        injective temporal graph attention module for computing temporal node embeddings of PINT.
        :param node_raw_features: Tensor, shape (num_nodes + 1, node_feat_dim)
        :param edge_raw_features: Tensor, shape (num_edges + 1, edge_feat_dim)
        :param neighbor_sampler: NeighborSampler, neighbor sampler
        :param time_encoder: TimeEncoder
        ...
        :param edge_feat_dim: int, dimension of edge features
        :param time_feat_dim:  int, dimension of time features (encodings)
        :param matrix_memory: MatrixMemory, matained temporal walk matrices
        :param device: str, pytorch device
        :param beta: float, time decay weight to aggregate neighbor nodes
        :param r_dim: int, dimension of relative encodings
        """
        ...
        self.r_dim = r_dim
        self.linear_1 = torch.nn.ModuleList([torch.nn.Linear(node_feat_dim +
                                                             edge_feat_dim + 2 * r_dim, node_feat_dim)
                                             for _ in range(num_layers)])
        self.linear_11 = torch.nn.ModuleList([torch.nn.Linear(node_feat_dim, node_feat_dim)
                                              for _ in range(num_layers)])
        self.linear_2 = torch.nn.ModuleList(
            [torch.nn.Linear(2 * node_feat_dim + 2 * r_dim,
                             node_feat_dim) for _ in range(num_layers)])

        self.linear_22 = torch.nn.ModuleList(
            [torch.nn.Linear(node_feat_dim,
                             node_feat_dim) for _ in range(num_layers)])
        self.alpha = 2
        self.beta = torch.nn.Parameter(torch.tensor(beta), requires_grad=False)

    def compute_node_temporal_embeddings(self, node_memories: torch.Tensor,
                                         node_ids: np.ndarray,
                                         node_interact_times: np.ndarray,
                                         current_layer_num: int, num_neighbors: int, src_node_ids: np.ndarray,
                                         dst_node_ids: np.ndarray):
        """
        given memory, node ids , and the corresponding time node_interact_times,
        return the temporal embeddings after convolution at the current_layer_num
        :param node_memories: Tensor, shape (num_nodes, memory_dim)
        :param node_ids: ndarray, shape (batch_size, ), node ids
        :param node_interact_times: ndarray, shape (batch_size, ), node interaction times
        :param current_layer_num: int, current layer number
        :param num_neighbors: int, number of neighbors to sample for each node
        :param src_node_ids, np.ndarray, shape (batch_size,), source node ids
        :param dst_node_ids, np.ndarray, shape (batch_size,), destination node ids
        """
        assert (current_layer_num >= 0)
        device = node_memories.device
        node_features = node_memories[torch.from_numpy(node_ids)] + self.node_raw_features[node_ids]
        ...
        if current_layer_num == 0:
            ...
            neighbor_embeddings = self.compute_node_temporal_embeddings(node_memories=node_memories,
                                                                        node_ids=neighbor_node_ids.flatten(),
                                                                        node_interact_times=neighbor_times.flatten(),
                                                                        current_layer_num=current_layer_num - 1,
                                                                        num_neighbors=num_neighbors,
